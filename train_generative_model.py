...
def train_model(name, g_train, d_train, sampler, generator, samples_per_epoch, nb_epoch,
    ...
    while epoch < nb_epoch:
      ...
      while samples_seen < samples_per_epoch:
        z, x = next(generator)
        print "z-------------!----------next(generator)\n",z.shape
        print "x-------------!----------next(generator)\n",x.shape
        print "counter-------!----------\n",counter
        # build batch logs
        batch_logs = {}
        if type(x) is list:
        ...
        if batch_index % 100 == 0:
          ...
          save_images(join_image, [8*2, 8],
                      '/data/self_driving/outputs/samples_%s/train_%s_%s.png' % (name, epoch, batch_index))
          ...
          for j, (i1, i2) in enumerate(zip(samples[:64], xs[:64])):
            ...
            join_image[j*2+1] = i2
          save_images(join_image, [8*2, 8],
                      '/data/self_driving/outputs/samples_%s/test_%s_%s.png' % (name, epoch, batch_index))
...
if __name__ == "__main__":
  ...
  print "\n\n\n\n----------train_generative_model.py start---------------\n"
  start = time.time()
  ...
  print "from models ", MODEL_NAME , " import get_model"
  ...
  if not os.path.exists("/data/self_driving/outputs/results_"+args.name):
      os.makedirs("/data/self_driving/outputs/results_"+args.name)
  if not os.path.exists("/data/self_driving/outputs/samples_"+args.name):
      os.makedirs("/data/self_driving/outputs/samples_"+args.name)

  with tf.Session() as sess:
    print "\n\n-----------------train_generative_model tf.Session()---------------\n"
    ...
    if args.loadweights:
      loader()
    print "\n\n-----------------train_generative_model train_model()---------------\n"
