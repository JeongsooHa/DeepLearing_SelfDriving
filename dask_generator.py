...
def concatenate(camera_names, time_len):
  print "------------------start dask_generator : concatenate-----------------"
...
def datagen(filter_files, time_len=1, batch_size=256, ignore_goods=False):
  ...
  """
  print "-----------------start dask_generator : datagen----------------"
  global first
  assert time_len > 0
  ...
  while True:
    try:
      ...
      while count < batch_size:
        ...
        for es, ee, x in c5x:
          if i >= es and i < ee:
            #print i,"   x[i-es-time_len+1:i-es+1] : ",i-es-time_len+1,"  ",i-es+1
      ...
      #print "angle_batch : "+str(angle_batch.shape)+"\n"+str(angle_batch)
      #print "speed_batch : "+str(speed_batch.shape)+"\n"+str(speed_batch)
