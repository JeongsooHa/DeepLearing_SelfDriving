...
if __name__ == "__main__":
  ...
  log = h5py.File("/data/self_driving/dataset/log/"+dataset+".h5", "r")
  cam = h5py.File("/data/self_driving/dataset/camera/"+dataset+".h5", "r")
  ...
  for i in range(skip*100, log['times'].shape[0]):
    ...
    print dataset," ",log['times'][i]
    print "predicted_steers : ",predicted_steers
    print "angle_steers : ",angle_steers
    print "speed_ms : ",speed_ms
    check.append(abs(angle_steers-predicted_steers)<10)
  
  #check.append(1==1)
  print "/n",check.count(True),"\n"
  print log['times'].shape[0]
