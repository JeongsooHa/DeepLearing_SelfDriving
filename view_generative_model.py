...
if __name__ == "__main__":
  ...
  with tf.Session() as sess:
    ...
    G.load_weights("/data/self_driving/outputs/results_autoencoder/G_weights.keras".format(args.name))
    E.load_weights("/data/self_driving/outputs/results_autoencoder/E_weights.keras".format(args.name))
    checkpoint_dir = '/data/self_driving/outputs/results_' + args.name
    T.load_weights(checkpoint_dir+"/T_weights.keras")

    if not os.path.exists("/data/self_driving/video_"+args.name):
      os.makedirs("/data/self_driving/video_"+args.name)
    ...
    cmd = "ffmpeg -y -i /data/self_driving/video_"+args.name+"/%03d.png /data/self_driving/video_"+args.name+"/output.gif -vf fps=1"
    print(cmd)
    os.system(cmd)
